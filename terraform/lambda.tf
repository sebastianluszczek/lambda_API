...
data "archive_file" "local_zip_getUsers" {
  source_file = "../${path.root}/dist/getUsers.js"
  type = "zip"
  output_path = "${path.root}/dist/getUsers.zip"
}

data "archive_file" "local_zip_getUser" {
  source_file = "${path.root}/dist/getUser.js"
  type = "zip"  
  output_path = "${path.root}/dist/getUser.zip"
}

data "archive_file" "local_zip_createUser" {
  source_file = "${path.root}/dist/createUser.js"
  type = "zip"
  output_path = "${path.root}/dist/createUser.zip"
}

data "archive_file" "local_zip_deleteUser" {
  source_file = "${path.root}/dist/deleteUser.js"
  type = "zip"
  output_path = "${path.root}/dist/deleteUser.zip"
}
...
resource "aws_s3_bucket_object" "zip_getUsers" {
  ...
  key = "getUsers.zip"
  source = "${path.root}/dist/getUsers.zip"
}

resource "aws_s3_bucket_object" "zip_getUser" {
  bucket = "${aws_s3_bucket.sls_api_storage.bucket}"
  key = "getUser.zip"
  source = "${path.root}/dist/getUser.zip"
}

resource "aws_s3_bucket_object" "zip_createUser" {
  bucket = "${aws_s3_bucket.sls_api_storage.bucket}"
  key = "createUser.zip"
  source = "${path.root}/dist/createUser.zip"
}

resource "aws_s3_bucket_object" "zip_deleteUser" {
  bucket = "${aws_s3_bucket.sls_api_storage.bucket}"
  key = "deleteUser.zip"
  source = "${path.root}/dist/deleteUser.zip"
}
